...
/**
 * Created by alan.baker on 10/24/17.
 */
...
public class GroupDiscoveryServiceImpl implements GroupDiscoveryService {
    ...
    private PublishSubject<SortedMap<String, Group>> discoveredGroupsPublishSubject = PublishSubject.create();

    public void start() {
        discoveredGroups.clear();

//        multicastThread = new Thread(new MulticastListen());
...
//        }
    ...
    @Override
    public Observable<SortedMap<String, Group>> getDiscoveredGroupsObservable() {
        return discoveredGroupsPublishSubject;
    }
            ...
            try {
                ...
                do {
                    ...
                    if (searchCounter < 4 && currentTimeMilis >= nextSearchTimeMillis) {
                        if (searchCounter == 3) {
                            updateDiscoveredGroupsPublishSubject();
                        } else {
...
//    /**
//     * Handles UDP multicast network messages.
//     */
...
//                socket.joinGroup(InetAddress.getByName(MULTICAST_SSDP_IP));
//                DatagramPacket receivePacket;
//                receivePacket = new DatagramPacket(new byte[1536], 1536);
//                while(!currentThread().isInterrupted()) {
...
//                Log.e(LOG_TAG, "There was an error communicating with the socket", exception);
//            } finally {
//                if (socket != null) {
//                    socket.disconnect();
//                    socket.close();
...
//    }
    ...
    private void updateDiscoveredGroupsPublishSubject() {
        discoveredGroupsPublishSubject.onNext(discoveredGroups);
    }
